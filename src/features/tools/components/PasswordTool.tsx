  ...
  return (
    <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="flex-1 relative">
            <input
              ...
              readOnly
              placeholder="点击生成按钮..."
              className="w-full px-3 sm:px-4 py-3 sm:py-4 text-lg sm:text-xl font-mono bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg text-center dark:text-white"
            />
          </div>
          <button
        ...
        {password && (
          <div className="mt-3 sm:mt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs sm:text-sm text-surface-500">密码强度</span>
              <span className={`text-xs sm:text-sm font-medium ${
                strength.color === 'red' ? 'text-red-500' :
                strength.color === 'yellow' ? 'text-yellow-500' :
                strength.color === 'blue' ? 'text-primary-500' :
                'text-green-500'
              }`}>
                {strength.label}
              </span>
            ...
            <div className="h-2 bg-surface-200 dark:bg-surface-700 rounded-full overflow-hidden">
              <div
                className={`h-full transition-all duration-500 ${
                  ...
                  strength.color === 'blue' ? 'bg-primary-500' :
                  'bg-green-500'
                }`}
      ...
      <div className="card p-4 sm:p-6 space-y-3 sm:space-y-4">
        <div className="flex items-center gap-2 sm:gap-4">
          <Settings2 className="w-4 h-4 sm:w-5 sm:h-5 text-surface-400" />
          <span className="text-sm sm:text-base font-medium text-surface-700 dark:text-surface-300">长度</span>
          <input
            type="range"
            id="password-length"
