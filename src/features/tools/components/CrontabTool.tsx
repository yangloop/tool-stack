  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="card p-4 sm:p-6 bg-gradient-to-r from-primary-50 to-indigo-50 dark:from-surface-800 dark:to-surface-900">
        <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4">
          <div className="flex-1">
            <div className="text-sm font-medium text-surface-700 dark:text-surface-300 mb-1 block">
              Cron 表达式
            </div>
            {isEditing ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 font-mono text-lg bg-surface-0 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg dark:text-white"
                  placeholder="* * * * *"
                />
                <button
              ...
              <div 
                ...
                className="font-mono text-2xl text-primary-600 dark:text-primary-400 cursor-pointer hover:underline"
              >
                {expression}
              </div>
            )}
        ...
        {/* 人类可读描述 */}
        <div className="mt-4 p-3 bg-white/50 dark:bg-surface-900/50 rounded-lg">
      ...
      <div className="card p-4 sm:p-6">
        <h3 className="font-medium text-surface-900 dark:text-surface-100 mb-4 flex items-center gap-2">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
          {cronFields.map((field, index) => (
            <div key={field.name}>
              <label htmlFor={`cron-field-${index}`} className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">
                {field.name}
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg text-sm dark:text-white"
              >
                <option value="*">*</option>
              ...
              <input
                ...
                onChange={(e) => handleValueChange(index, e.target.value)}
                placeholder="自定义"
                className="w-full mt-2 px-3 py-1.5 text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg dark:text-white"
              />
            </div>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
          {presets.map((preset) => (
            ...
            >
              ...
              <div className="font-mono text-xs text-primary-600 dark:text-primary-400 mt-1">
                {preset.value}
              </div>
