...
export function HttpRequestTool() {
  ...
  const [urlEncodedData, setUrlEncodedData] = useState<Param[]>([
  ...
  // 解析 URL 参数 - 只在组件挂载时执行一次
  const hasParsedUrlRef = useRef(false);
  useEffect(() => {
    if (hasParsedUrlRef.current) return;
    hasParsedUrlRef.current = true;

    const { baseUrl: parsedBase, params: parsedParams } =
      parseUrlParams(baseUrl);
  ...
  const getLanguageFromContentType = (contentType: string): string => {
    ...
    if (type.includes("application/x-yaml") || type.includes("text/yaml"))
      return "yaml";
    if (type.includes("application/sql") || type.includes("text/sql"))
      return "sql";
  ...
  const handleCurlImport = (result: CurlParseResult) => {
    ...
    // 解析 URL 参数
    const { baseUrl: parsedBase, params: parsedParams } = parseUrlParams(
      result.url,
    );
    setBaseUrl(parsedBase);
    if (parsedParams.length > 0) {
    ...
    // 设置请求头
    const newHeaders: Header[] = Object.entries(result.headers).map(
      ([key, value]) => ({
        key,
        value,
        enabled: true,
      ...
      <div className="card p-3 sm:p-4 mb-4 sm:mb-5">
        <div className="flex gap-2 sm:gap-3">
          ...
          {loading ? (
            ...
            >
              <svg
                className="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin flex-shrink-0"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
            ...
            {responseTab === "body" ? (
              ...
              <div className="space-y-2 max-h-96 overflow-auto">
                {Object.entries(response.headers).map(([key, value]) => (
                  <div
                    key={key}
                    className="flex gap-3 sm:p-4 text-sm py-2 border-b border-surface-100 dark:border-surface-700/50 last:border-0"
                  >
                    <span className="font-medium text-surface-700 dark:text-surface-300 min-w-[180px] flex-shrink-0">
                      {key}:
                    </span>
                    <span className="text-surface-600 dark:text-surface-400 break-all font-mono text-xs">
                      {value}
                    </span>
                  </div>
                ))}
        ...
        <div className="card mb-4 sm:mb-5">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="flex items-center gap-2">
              <History className="w-4 h-4 sm:w-5 sm:h-5 text-surface-500" />
...
function MethodBadge({ method }: { method: string }) {
  ...
  return (
    <span
      className={`px-2 py-1 text-[10px] font-semibold rounded-lg ${colorMap[method] || colorMap["GET"]}`}
    >
      {method}
    </span>
  );
