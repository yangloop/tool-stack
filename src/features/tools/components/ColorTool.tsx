  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="card p-4 sm:p-6 mb-4">
        <div
          className="w-full h-24 sm:h-32 rounded-lg border border-surface-200 dark:border-surface-700"
      ...
      {/* 预设颜色 */}
      <div className="card p-4 sm:p-6 mb-4">
        <div className="block text-xs sm:text-sm font-medium text-surface-700 dark:text-surface-300 mb-2 sm:mb-3">
          预设颜色
        </div>
        <ColorSwatch 
          colors={presetColors}
      ...
      <div className="grid md:grid-cols-3 gap-3 sm:gap-4">
        ...
        {/* RGB */}
        <div className="card p-4 sm:p-6 space-y-3">
          <div className="font-medium text-sm sm:text-base text-surface-700 dark:text-surface-300">RGB</div>
          <div className="space-y-2">
            {(['r', 'g', 'b'] as const).map((key) => (
              <div key={key} className="flex items-center gap-2">
                <span className="w-4 text-xs sm:text-sm text-surface-500 uppercase">{key}</span>
                <input
                  type="number"
                  id={`color-rgb-${key}`}
                  name={`color-rgb-${key}`}
                  ...
                  className="flex-1 px-2 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg dark:text-surface-100"
                />
              </div>
            ))}
          </div>
        ...
        {/* HSL */}
        <div className="card p-4 sm:p-6 space-y-3">
          <div className="font-medium text-sm sm:text-base text-surface-700 dark:text-surface-300">HSL</div>
          <div className="space-y-2">
            {(['h', 's', 'l'] as const).map((key) => (
              <div key={key} className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="flex-1 px-2 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg dark:text-surface-100"
                />
                <span className="text-xs sm:text-sm text-surface-400">{key === 'h' ? '°' : '%'}</span>
              </div>
            ))}
          </div>
        </div>
