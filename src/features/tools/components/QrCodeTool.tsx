      ...
      <div className="grid lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="card p-4 sm:p-6 space-y-4 sm:space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div>
              <label htmlFor="qr-size" className="block text-xs sm:text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">
                尺寸
              </label>
              <select
                id="qr-size"
                ...
                className="w-full px-2 sm:px-3 py-2 bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg text-xs sm:text-sm dark:text-white"
              >
            ...
            <div>
              <label htmlFor="qr-error-correction" className="block text-xs sm:text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">
                纠错级别
              </label>
              <select
          ...
          {/* Logo 设置 */}
          <div className="border border-surface-200 dark:border-surface-700 rounded-lg p-3 sm:p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs sm:text-sm font-medium text-surface-700 dark:text-surface-300">Logo</span>
              {logoImage && (
                <button
                  onClick={removeLogo}
            ...
            {logoImage ? (
              <div className="flex items-center gap-3">
                <div className="w-16 h-16 border border-surface-200 dark:border-surface-700 rounded-lg overflow-hidden flex-shrink-0">
                  <img
                    src={logoImage.src}
                    alt="Logo预览"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-surface-500 dark:text-surface-400 mb-2">已上传 Logo</p>
                  <button
              ...
              <div
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-surface-300 dark:border-surface-600 rounded-lg p-4 text-center cursor-pointer hover:border-primary-500 dark:hover:border-primary-400 transition-colors"
              >
                <Upload className="w-6 h-6 mx-auto mb-2 text-surface-400" />
                <p className="text-xs text-surface-500 dark:text-surface-400">点击上传 Logo</p>
                <p className="text-xs text-surface-400 mt-1">支持 JPG、PNG、GIF，最大 2MB</p>
              </div>
            )}
            ...
            {logoImage && (
              <div className="pt-2 border-t border-surface-200 dark:border-surface-700">
                <div className="flex items-center justify-between mb-2">
                  <label htmlFor="qrcode-logo-size" className="text-xs text-surface-600 dark:text-surface-400">尺寸</label>
                  <span className="text-xs text-surface-500">{logoSizePercent}%</span>
                </div>
                <input
                  type="range"
                  id="qrcode-logo-size"
                ...
                {logoSizePercent >= 18 && errorCorrection !== 'H' && (
                  <div className="mt-2 flex items-start gap-1.5 text-xs text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 p-2 rounded">
                    <span>
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <span className="text-xs sm:text-sm font-medium text-surface-700 dark:text-surface-300">颜色设置</span>
          ...
          {text.trim() ? (
            <>
              <div className="max-w-full overflow-auto flex justify-center">
                <canvas
                  ref={canvasRef}
                  className="border border-surface-200 dark:border-surface-700 rounded-lg shadow-sm"
