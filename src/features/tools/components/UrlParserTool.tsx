...
function InfoItem({ 
  ...
  return (
    <div className={`p-2 sm:p-3 rounded-lg ${highlight ? 'bg-blue-50 dark:bg-blue-900/20' : 'bg-surface-50 dark:bg-surface-800'}`}>
      ...
      <div className="mt-1 flex items-center gap-2">
        {link ? (
          <a 
            ...
            className="font-mono text-xs sm:text-sm text-primary-600 dark:text-primary-400 hover:underline break-all"
          >
            {value}
          </a>
        ) : (
          <code className={`font-mono text-xs sm:text-sm break-all ${highlight ? 'text-blue-700 dark:text-blue-300' : 'text-surface-800 dark:text-surface-200'}`}>
            {value}
          </code>
        )}
        {link && <ExternalLink className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-surface-400 flex-shrink-0" />}
      </div>
    </div>
  );
}
    ...
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="card p-4 sm:p-6 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <label htmlFor="url-input" className="text-xs sm:text-sm font-medium text-surface-700 dark:text-surface-300">
            URL 地址
          </label>
          <button
        ...
        <input
          ...
          className="w-full px-3 sm:px-4 py-2 sm:py-3 font-mono text-xs sm:text-sm bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg focus:ring-2 focus:ring-primary-500 dark:text-white"
        />
        ...
        <div className="flex flex-wrap gap-2">
          <span className="text-xs text-surface-500">示例:</span>
      ...
      {parsed && (
        <>
          ...
          <div className="card p-4 sm:p-6">
            <h3 className="font-medium text-sm sm:text-base text-surface-900 dark:text-surface-100 mb-3 sm:mb-4 flex items-center gap-2">
              <Layers className="w-4 h-4 sm:w-5 sm:h-5 text-primary-500" />
              URL 结构
            </h3>
            
            {/* 可视化分解 */}
            <div className="p-2 sm:p-4 bg-surface-50 dark:bg-surface-900 rounded-lg font-mono text-xs sm:text-sm overflow-x-auto">
              <div className="flex flex-wrap items-center gap-1">
                ...
                {/* 锚点 */}
                {parsed.hash && (
                  <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-gray-200 dark:bg-gray-700 text-surface-700 dark:text-surface-300 rounded text-xs sm:text-sm">
                    #{parsed.hash.slice(1)}
                  </span>
                )}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-3 sm:gap-4">
            ...
            <div className="card p-4 sm:p-6 space-y-2 sm:space-y-3">
              <h3 className="font-medium text-sm sm:text-base text-surface-900 dark:text-surface-100 flex items-center gap-2">
            ...
            <div className="card p-4 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3 sm:mb-4">
                <h3 className="font-medium text-sm sm:text-base text-surface-900 dark:text-surface-100 flex items-center gap-2">
                  ...
                  查询参数
                  <span className="text-xs sm:text-sm text-surface-500">({params.length} 个)</span>
                </h3>
                ...
                <table className="w-full text-xs sm:text-sm">
                  <thead>
                    <tr className="border-b border-surface-200 dark:border-surface-700">
                      <th className="text-left py-2 px-2 sm:px-3 text-surface-500 font-medium">序号</th>
                      <th className="text-left py-2 px-2 sm:px-3 text-surface-500 font-medium">参数名</th>
                      <th className="text-left py-2 px-2 sm:px-3 text-surface-500 font-medium">值</th>
                      <th className="text-right py-2 px-2 sm:px-3 text-surface-500 font-medium">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {params.map((param, idx) => (
                      <tr key={idx} className="border-b border-surface-100 dark:border-surface-800 last:border-0">
                        <td className="py-2 px-2 sm:px-3 text-surface-400">{idx + 1}</td>
                        <td className="py-2 px-2 sm:px-3">
                          <code className="bg-surface-100 dark:bg-surface-800 px-1.5 sm:px-2 py-0.5 rounded text-primary-600 dark:text-primary-400 text-xs">
                            {showDecoded ? param.decodedKey : param.key}
                          </code>
                        </td>
                        <td className="py-2 px-2 sm:px-3">
                          <code className="bg-surface-100 dark:bg-surface-800 px-1.5 sm:px-2 py-0.5 rounded break-all text-xs">
          ...
          <div className="card p-4 sm:p-6">
            ...
            >
              <span className="font-medium text-sm sm:text-base text-surface-900 dark:text-surface-100 flex items-center gap-2">
                <Code className="w-4 h-4 sm:w-5 sm:h-5 text-surface-500" />
                原始数据 (JSON)
              </span>
              {showRaw ? (
                <span className="text-xs text-primary-500">收起</span>
              ) : (
                <span className="text-xs text-primary-500">展开</span>
              )}
            </button>
            {showRaw && (
              <pre className="mt-3 p-2 sm:p-3 bg-surface-50 dark:bg-surface-900 rounded-lg text-xs font-mono text-surface-700 dark:text-surface-300 overflow-x-auto">
