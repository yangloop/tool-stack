...
function CompressedView({ data, isDark }: { data: object; isDark: boolean }) {
  ...
  return (
    <div className={`p-3 sm:p-4 min-h-full ${isDark ? 'bg-surface-900' : 'bg-surface-50'}`} style={{ minWidth: 0 }}>
      <pre 
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-surface-900 dark:text-surface-100 flex items-center gap-2">
          <FileJson className="w-6 h-6 sm:w-7 sm:h-7 text-primary-500" />
          JSON 工具
        </h1>
      ...
      <div className="grid lg:grid-cols-2 gap-3 sm:gap-4">
        ...
        <div className="card p-4 sm:p-6 min-w-0">
          <div className="flex items-center justify-between mb-2 sm:mb-3 min-h-[36px]">
            ...
            <div className="invisible flex gap-2">
              <div className="flex bg-surface-100 dark:bg-surface-800 rounded p-0.5">
        ...
        <div className="card p-4 sm:p-6 min-w-0">
          ...
          <div className="h-[400px] bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg overflow-auto">
            {parseResult.data ? (
              viewMode === 'formatted' ? (
                <FormattedView data={parseResult.data as object} isDark={isDark} />
              ) : (
                <CompressedView data={parseResult.data as object} isDark={isDark} />
              )
            ) : (
