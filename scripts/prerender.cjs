...
const toolsConfig = {
  ...
  'text-diff': {
    name: '文本对比',
    description: '免费在线文本对比工具。基于 Monaco Diff Editor 提供接近 VS Code 的代码 diff 体验，支持行内对比、差异统计、定位跳转、一键格式化和 JSON 重排。',
    keywords: '文本对比,文本比较,Monaco Diff,代码对比,差异高亮,JSON对比,SQL对比,VSCode diff,代码对比工具',
    category: '开发调试',
    faq: [
      { q: '文本对比工具有什么用？', a: '可以比较两段文本或代码的差异，常用于代码审查、配置核对、接口响应比对和文档版本检查。' },
      { q: '支持哪些对比模式？', a: '支持双栏和行内 diff，能够查看新增、删除、修改以及一行内部的细粒度变化。' },
      { q: '可以对比代码吗？', a: '可以，尤其适合对比 JSON、SQL、TypeScript、HTML、XML、YAML、配置文件和脚本内容。' }
    ],
    howTo: {
      steps: [
        { name: '输入两段内容', text: '在左右编辑器中分别粘贴旧版本和新版本文本或代码。' },
        { name: '格式化并切换视图', text: '根据内容类型选择语言，必要时先一键格式化或执行 JSON 重排，再切换双栏或行内视图。' },
        { name: '统计并定位差异', text: '结合差异统计和上一个/下一个跳转按钮，快速定位每一处实际变更。' }
      ]
    }
  },
  'text-template': {
